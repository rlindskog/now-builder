...
const { runNpmInstall } = require('@now/build-utils')
...
function startDevServer (rootDir, nuxtDep) {
  const NUXT_DEP_NAME = nuxtDep.name
  const encodedEnv = Buffer.from(JSON.stringify({ NODE_ENV: 'development', NUXT_DEP_NAME })).toString('base64');
...
async function build({ files, entrypoint, workPath, config = {}, meta }) {
  ...
  const name = '@nuxt/now-builder'
  // Read package.json
  let pkg = await readPackageJson(rootDir)
  ...
  if (meta.isDev) {
    ...
    if (!urls[entrypoint]) {
      startStep(`${name} Installing dependencies...`)
      await runNpmInstall(rootDir, ['--prefer-offline'])
      const { forked, getUrl } = startDevServer(rootDir, nuxtDependency)
      urls[entrypoint] = await getUrl()
      childProcess = forked
      console.log(`${name} Development server for ${entrypoint} running at ${urls[entrypoint]}`)

      const pathsInside = getPathsInside(entryDir, files);
      return {
        output: {},
        // routes: getRoutes(entryDir, pathsInside, files, urls[entrypoint]),
        watch: pathsInside,
